...
from dataloaders.dataset import (BaseDataSets, RandomGenerator,
                                 TwoStreamBatchSampler, MyDataSets)
from networks.net_factory import net_factory
from utils_ import losses, metrics, ramps
from val_2D import test_single_volume, test_single_volume_my
...
parser.add_argument('--patch_size', type=list,  default=[320, 512],
                    help='patch size of network input')
parser.add_argument('--seed', type=int,  default=67373, help='random seed')
...
def train(args, snapshot_path):
    ...
    db_train = MyDataSets(base_dir=args.root_path, split="train", num=None)
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            if epoch_num > 40000:
                for i in range(batch_size):
                    # 以日期命名保存伪标签
                    # 过滤伪标签
                    if filter_pseudo_labels(pseudo_outputs1,'./model',0.8):
                        torchvision.utils.save_image(pseudo_outputs1, f'{volume_batch[i]}pseudo_label.png')
